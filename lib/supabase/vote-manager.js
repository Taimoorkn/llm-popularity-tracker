...
class SupabaseVoteManager {
  ...
  // Enhanced getVoteCounts - now returns detailed stats
  async getVoteCounts() {
    try {
      const { data, error } = await supabase
        .from('vote_stats_aggregate')
        .select('llm_id, total_votes, upvotes, downvotes, unique_voters')
      ...
      const voteStats = {}
      data.forEach(item => {
        voteStats[item.llm_id] = {
          total: item.total_votes || 0,
          upvotes: item.upvotes || 0,
          downvotes: item.downvotes || 0,
          uniqueVoters: item.unique_voters || 0
        }
      })
  ...
  async syncUserData(fingerprint) {
    try {
      ...
      const voteStats = await this.getVoteCounts()
      ...
      return {
        success: true,
        userVotes,
        voteStats: voteStats,  // Now contains detailed stats
        rankings,
        stats
      }
    } catch (error) {
